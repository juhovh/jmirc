...
import net.s4bb.customization.TextBox_Form;
import net.s4bb.customization.TextBox_Form_Listener;
...
public class Window extends Canvas implements CommandListener {
	...
	private static String WRITE_TEXT, JOIN_CHANNEL, CHANGE_NICK, OPEN_QUERY, ADD_FAVORITE;
...
//		textbox = new TextBox(CHANGE_NICK, "", 30, TextField.ANY);
    textbox = new TextBox_Form(CHANGE_NICK, "", 30, TextField.ANY);
    textbox.setTextBox_Form_Listener(new TextBox_Form_ListenerImpl());
...
//			textbox = new TextBox(WRITE_TEXT, null, 512, TextField.ANY);
      textbox = new TextBox_Form(WRITE_TEXT, null, 512, TextField.ANY);
      textbox.setTextBox_Form_Listener(new TextBox_Form_ListenerImpl());
	...
	public void commandAction(Command c, Displayable s) {
		if (c == cmd_msg || c == cmd_join || c == cmd_query) {
			if (c == cmd_msg) {
//				textbox = new TextBox(WRITE_TEXT,"",1000,TextField.ANY);
        textbox = new TextBox_Form(WRITE_TEXT,"",1000,TextField.ANY);
        textbox.setTextBox_Form_Listener(new TextBox_Form_ListenerImpl());
				textbox.addCommand(cmd_send);
			}
			else {
//				textbox = new TextBox(c == cmd_join ? JOIN_CHANNEL : OPEN_QUERY,"",100,TextField.ANY);
        textbox = new TextBox_Form(c == cmd_join ? JOIN_CHANNEL : OPEN_QUERY,"",100,TextField.ANY);
        textbox.setTextBox_Form_Listener(new TextBox_Form_ListenerImpl());
				textbox.addCommand(cmd_ok);
			}
			textbox.addCommand(cmd_cancel);
			...
			jmIrc.disconnect("QUIT :used " + jmIrc.NAME + "\r\n");
...
//			textbox = new TextBox(ADD_FAVORITE, "", 128, TextField.ANY);
      textbox = new TextBox_Form(ADD_FAVORITE, "", 128, TextField.ANY);
      textbox.setTextBox_Form_Listener(new TextBox_Form_ListenerImpl());
			textbox.setCommandListener(new TextboxListener());
			textbox.addCommand(cmd_ok);
			textbox.addCommand(cmd_cancel);
    ...
    public void commandAction(Command c, Displayable s) {
      ...
      if (c == cmd_send){
        ...
        if (tbtitle.equals(JOIN_CHANNEL)) {
          ...
          Vector list = uihandler.getFavs();
          while (cg_favourites.size() > 0)
            cg_favourites.delete(0);
          for (int i = 0; i < list.size(); i++)
            cg_favourites.append((String) list.elementAt(i), null);
          uihandler.setDisplay(favform);
        }
  ...
  private class TextBox_Form_ListenerImpl implements TextBox_Form_Listener{
    public void textEntered(TextBox_Form tbf, String text){
      uihandler.setWinlock(false);
      String tbtitle = tbf.getTitle();
      String tbstring = text;
      textbox = null;
      
      if(text!=null){
        if (tbtitle.equals(WRITE_TEXT)){
          handleMsg(text);
          show();
        }
        else{
